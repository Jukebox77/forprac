...
import pyperclip as cb  # 클립보드를 제어할 수 있는 간편한 툴입니다. pip 로 설치하세요.
...
if __name__ == '__main__':
    ...
    hwp.RegisterModule("FilePathCheckDLL", "SecurityModule")  # 보안승인모듈 활성화
    hwp.XHwpWindows.Item(0).Visible = True
    hwp.Open(os.path.join(os.getcwd(), "별헤는밤_원본.hwp"))  # 원본 열고,
    hwp.Run("FileNew")  # 또 새 창,
    hwp.Open(os.path.join(os.getcwd(), "별헤는밤_조작.hwp"))  # 조작본 열고,
    hwp.Run("FileNew")  # 또 새 창,
    hwp.Open(os.path.join(os.getcwd(), "비교표.hwp"))  # 비교표 열고,
    ...
    while stop_signal:
        ...
        if signal == 1:  # 문서 마지막에 도착하면 GetText 에서 반환한 튜플값 첫번째 값이 1입니다.
            break  # 그러면 while 문 종료.
    hwp.ReleaseScan()  # InitScan 후에는 꼭 ReleaseScan 을 실행해주셔야 합니다. Open 과 Close 처럼요.
    original_full_text = original_full_text.split('\r\n')[:-1]  # 문서 마지막의 엔터가 리스트의 마지막 원소 '\r\n' 제거.
    ...
    for original_statement in original_full_text:  # 원본 문장을 전부 순회하면서
        ...
        if max_ratio < 1.0:  # 완전히 똑같지 않다면?
            ...
            글자색('black')  # 그렇지 않다면 다시 까맣게. 기본적으로 위쪽 셀의 글자색이 아래 셀에 유지됨. 버그는 아닌듯?

        hwp.Run('TableRightCellAppend')  # 탭 키를 눌러서 아래에 새 행을 생성. 원본 문장 수만큼 반복!

# 원본.SetActive_XHwpDocument()  # 원본 활성화
# 원본.Close()
# 사본.SetActive_XHwpDocument()  # 원본 활성화
# 사본.Close()



    # hwp.SaveAs(os.path.join(os.environ["USERPROFILE"], "desktop", "최종본.hwp"))  # 바탕화면에 최종 비교표를 저장함.
    # hwp.Quit()